...
VALUES (nextval('id_seq_plugin_step_variable'),(SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.name='Copacetic v1.0.0' and ps."index"=1 and ps.deleted=false),'Timeout','STRING','Timeout for copa patch command, default timeout is 5 minutes.','t','t','5m',null,'INPUT','NEW',null,1,null,null,'f','now()',1,'now()',1);